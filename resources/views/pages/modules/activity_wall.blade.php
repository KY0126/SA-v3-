...
<div class="space-y-6">
  <div class="flex items-center justify-between flex-wrap gap-2">
    ...
    <div class="flex gap-2">
      ...
      <input id="act-search" oninput="applyActFilters()" placeholder="搜尋活動..." class="px-3 py-1.5 rounded-fju border border-gray-200 text-xs w-36">
...
function applyActFilters(){
  ...
  const s=(document.getElementById('act-search')?.value||'').toLowerCase();
  if(s) data=data.filter(a=>a.title.toLowerCase().includes(s)||(a.description||'').toLowerCase().includes(s));
...
function renderActs(data){document.getElementById('act-grid').innerHTML=data.length===0?'<div class="p-8 text-center text-gray-400 col-span-2">無活動</div>':data.map(a=>'<div class="bg-white rounded-fju-lg p-5 shadow-sm border border-gray-100 card-hover"><div class="flex items-start justify-between mb-2"><h3 class="font-bold text-fju-blue">'+a.title+'</h3><span class="px-2 py-1 rounded-fju text-xs '+(a.status==='approved'?'bg-fju-green/10 text-fju-green':'bg-fju-yellow/20 text-fju-yellow')+'">'+a.status+'</span></div><p class="text-xs text-gray-500 mb-2">'+(a.description||'')+'</p><div class="flex items-center gap-4 text-xs text-gray-400"><span><i class="fas fa-calendar mr-1"></i>'+a.event_date+'</span><span><i class="fas fa-clock mr-1"></i>'+a.start_time+'-'+a.end_time+'</span><span><i class="fas fa-users mr-1"></i>'+a.current_participants+'/'+a.max_participants+'</span></div><div class="flex gap-2 mt-3"><button onclick="registerAct('+a.id+')" class="btn-yellow px-3 py-1 text-xs">報名</button><button onclick="deleteAct('+a.id+')" class="text-fju-red text-xs px-2 py-1 rounded hover:bg-fju-red/10"><i class="fas fa-trash"></i></button></div></div>').join('')}
