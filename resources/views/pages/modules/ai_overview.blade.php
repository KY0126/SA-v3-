...
<div class="space-y-6">
  <div class="flex items-center justify-between flex-wrap gap-2">
    ...
    <select id="ai-role-view" onchange="switchAIRole(this.value)" class="px-3 py-1.5 rounded-fju border border-gray-200 text-xs">
      ...
      <option value="admin" {{ $currentRole === 'admin' ? 'selected' : '' }}>課指組/處室</option>
  ...
  <div class="bg-white rounded-fju-lg p-6 shadow-sm border border-gray-100">
    ...
    <div class="grid md:grid-cols-3 gap-4 mb-4">
      <div class="p-4 rounded-fju bg-fju-blue/5 border border-fju-blue/10 text-center">
        ...
        <p class="text-xs text-gray-500 mt-1">Dify RAG + Embedding 技術比對 8 項校內法規（活動申請辦法、場地管理規則、安全管理規範等），自動判斷申請是否合規。</p>
      ...
      <div class="p-4 rounded-fju bg-fju-yellow/5 border border-fju-yellow/10 text-center">
        ...
        <p class="text-xs text-gray-500 mt-1">三層防護架構：規則引擎自動過濾 → RAG 語義比對法規 → GPT-4 深度分析。每次決策記錄於 ai_review_logs。</p>
      ...
      <div class="p-4 rounded-fju bg-fju-green/5 border border-fju-green/10 text-center">
        ...
        <p class="text-xs text-gray-500 mt-1">場地衝突時 AI 分析雙方歷史、優先權、替代方案，產出 3 個方案並計算信心度，協助雙方快速達成共識。</p>
    ...
    <div class="p-3 rounded-fju bg-fju-bg text-xs text-gray-500">
      ...
      <b>設計邏輯：</b>三層防護 — 第一層規則引擎自動過濾，第二層 RAG 語義比對法規，第三層 GPT-4 深度分析。每次 AI 決策均有完整審計日誌記錄於 ai_review_logs 資料表。
    </div>
  </div>

  {{-- Role-based AI Features (with working implementations) --}}
  ...
  <div class="bg-white rounded-fju-lg p-6 shadow-sm border border-gray-100">
    ...
    <div id="demo-area" class="min-h-[200px] rounded-fju bg-fju-bg p-4 relative overflow-hidden">
      <div id="demo-content" class="space-y-3"><p class="text-center text-gray-400 text-sm py-8">請選擇上方按鈕開始 AI 功能演示</p></div>
    </div>
  </div>

  {{-- AI Proposal Generator --}}
  <div class="bg-white rounded-fju-lg p-6 shadow-sm border border-gray-100">
    <h3 class="font-bold text-fju-blue mb-4"><i class="fas fa-wand-magic-sparkles mr-2 text-fju-yellow"></i>AI 企劃生成器（實作）</h3>
    <div class="space-y-3">
  ...
  {{-- AI Pre-Review (working) --}}
  <div class="bg-white rounded-fju-lg p-6 shadow-sm border border-gray-100">
    <h3 class="font-bold text-fju-blue mb-4"><i class="fas fa-search mr-2 text-fju-yellow"></i>AI 預審檢查（實作）</h3>
  ...
  {{-- AI Activity Recommendation (working for student) --}}
  <div class="bg-white rounded-fju-lg p-6 shadow-sm border border-gray-100">
  ...
  {{-- AI Credit Warning (working for admin) --}}
  <div class="bg-white rounded-fju-lg p-6 shadow-sm border border-gray-100">
  ...
  {{-- AI Service Monitor (working for IT) --}}
  <div class="bg-white rounded-fju-lg p-6 shadow-sm border border-gray-100">
...
const aiFeatures={
  admin:[
    {icon:'fa-chart-bar',name:'審核統計看板',desc:'即時查看所有 AI 預審通過率、風險分布、審核時間等統計數據，支援匯出報表。',color:'#003153',action:'showAdminDashboard()'},
    {icon:'fa-gavel',name:'RAG 法規比對',desc:'批量上傳企劃書，AI 自動比對 8 項校內法規並標記不合規條款，提供修改建議。',color:'#DAA520',action:'window.location.href="/module/rag-search?role=admin"'},
    {icon:'fa-user-check',name:'信用預警系統',desc:'AI 監控全校社團信用積分，自動發送預警通知，防止信用崩潰。',color:'#008000',action:'checkCreditWarnings()'},
    {icon:'fa-robot',name:'自動核准建議',desc:'對低風險案件（人數≤50、預算合理），AI 自動產出核准建議，加速審批流程。',color:'#004070',action:'doPreReview()'}
  ],
  officer:[
    {icon:'fa-file-alt',name:'企劃書 AI 生成',desc:'輸入活動基本資訊，AI 自動生成含預算、流程、SDGs、風險評估的完整企劃書。',color:'#003153',action:'document.getElementById("pg-title").scrollIntoView({behavior:"smooth"})'},
    {icon:'fa-search',name:'AI 預審自檢',desc:'送出前先透過 AI 預審系統自我檢查，確保通過率。系統標示需修改項目。',color:'#DAA520',action:'doPreReview()'},
    {icon:'fa-handshake',name:'衝突 AI 協商',desc:'場地衝突時 AI 提供三方案供選擇，含信心度評分和替代建議。',color:'#008000',action:'runDemo("conflict")'}
  ],
  professor:[
    {icon:'fa-clipboard-check',name:'審查輔助',desc:'AI 自動摘要學生提交的企劃書，標示重點和潛在風險，協助快速審查。',color:'#003153',action:'doPreReview()'},
    {icon:'fa-chart-line',name:'社團表現分析',desc:'AI 分析指導社團的活動成效、參與率、預算執行率等指標趨勢。',color:'#DAA520',action:'showProfessorAnalysis()'}
  ],
  student:[
    {icon:'fa-lightbulb',name:'活動推薦',desc:'根據過往參與記錄和興趣標籤，AI 推薦最適合的社團活動。',color:'#003153',action:'getRecommendations()'},
    {icon:'fa-search',name:'法規查詢助手',desc:'用自然語言詢問校內活動相關法規，AI 搜尋法規庫並回答。',color:'#DAA520',action:'window.location.href="/module/rag-search?role=student"'},
    {icon:'fa-file-alt',name:'企劃書 AI 生成',desc:'填入活動概要，AI 自動產出規格完整的企劃書草稿。',color:'#008000',action:'document.getElementById("pg-title").scrollIntoView({behavior:"smooth"})'}
  ],
  it:[
    {icon:'fa-server',name:'AI 服務監控',desc:'監控 AI API 回應時間、成功率、Token 使用量，確保服務穩定。',color:'#003153',action:'loadAIMonitor()'},
    {icon:'fa-shield-alt',name:'WAF + 安全分析',desc:'AI 分析 API 呼叫模式，偵測異常行為，自動觸發 WAF 防護。',color:'#C0392B',action:'loadAIMonitor()'},
    {icon:'fa-database',name:'資料品質監控',desc:'AI 定期掃描資料庫一致性，偵測髒資料並產出清理建議。',color:'#DAA520',action:'loadAIMonitor()'}
  ]
};
...
function showPrereviewDemo(a){animateSteps(a,['<div class="flex items-center gap-2 p-2 rounded bg-fju-blue/10 animate-fadeIn"><i class="fas fa-file-upload text-fju-blue"></i><span class="text-sm">收到企劃書：「攝影社春季外拍」，預計 60 人</span></div>','<div class="flex items-center gap-2 p-2 rounded bg-fju-yellow/10 animate-fadeIn mt-2"><i class="fas fa-cog fa-spin text-fju-yellow"></i><span class="text-sm">Step 1: 規則引擎 — 人數 60 ≤ 80 ✓ | 預算 18,000 ✓</span></div>','<div class="flex items-center gap-2 p-2 rounded bg-fju-blue/10 animate-fadeIn mt-2"><i class="fas fa-search text-fju-blue"></i><span class="text-sm">Step 2: RAG 法規比對 — 匹配「校園活動安全管理辦法」第 12 條</span></div>','<div class="flex items-center gap-2 p-2 rounded bg-fju-green/10 animate-fadeIn mt-2"><i class="fas fa-robot text-fju-green"></i><span class="text-sm">Step 3: GPT-4 深度分析 — 風險等級: low</span></div>','<div class="mt-3 p-3 rounded-fju bg-fju-green/10 border border-fju-green/20 animate-fadeIn"><div class="flex items-center gap-2"><i class="fas fa-check-circle text-fju-green text-xl"></i><div><span class="font-bold text-fju-green">AI 預審通過</span><br><span class="text-xs text-gray-500">風險等級: 低 | 信心度: 92% | 審核時間: 1.3s</span></div></div></div>'],800)}
function showProposalDemo(a){animateSteps(a,['<div class="flex items-center gap-2 p-2 rounded bg-fju-blue/10 animate-fadeIn"><i class="fas fa-keyboard text-fju-blue"></i><span class="text-sm">輸入：「程式設計工作坊」40 人</span></div>','<div class="flex items-center gap-2 p-2 rounded bg-fju-yellow/10 animate-fadeIn mt-2"><i class="fas fa-cog fa-spin text-fju-yellow"></i><span class="text-sm">匹配歷史成功企劃書模板...</span></div>','<div class="flex items-center gap-2 p-2 rounded bg-fju-blue/10 animate-fadeIn mt-2"><i class="fas fa-file-alt text-fju-blue"></i><span class="text-sm">生成預算: 場地 $3,000 + 講師 $5,000 + 材料 $4,000 = $12,000</span></div>','<div class="mt-3 p-3 rounded-fju bg-fju-yellow/10 border border-fju-yellow/20 animate-fadeIn"><b class="text-fju-blue">AI 企劃書摘要</b><br><span class="text-sm">一、宗旨：培養程式能力<br>二、地點：SF 134<br>三、預算：NT$12,000<br>四、SDGs：SDG4 (優質教育)<br>五、AI 風險評估：低風險 (89%)</span></div>'],800)}
function showConflictDemo(a){animateSteps(a,['<div class="flex items-center gap-2 p-2 rounded bg-fju-red/10 animate-fadeIn"><i class="fas fa-exclamation-triangle text-fju-red"></i><span class="text-sm">衝突偵測：攝影社 vs 吉他社 — 中美堂 04/15</span></div>','<div class="flex items-center gap-2 p-2 rounded bg-fju-yellow/10 animate-fadeIn mt-2"><i class="fas fa-cog fa-spin text-fju-yellow"></i><span class="text-sm">分析歷史預約、優先權、替代方案...</span></div>','<div class="mt-2 space-y-1 animate-fadeIn"><div class="flex items-center gap-2 p-2 rounded bg-fju-green/10 border border-fju-green/20"><span class="w-6 h-6 rounded-full bg-fju-green text-white flex items-center justify-center text-xs font-bold">1</span><span class="text-sm flex-1">攝影社改至 SF 134</span><span class="text-xs px-2 py-0.5 rounded-full bg-fju-green/20 text-fju-green">85%</span></div><div class="flex items-center gap-2 p-2 rounded bg-white border border-gray-100"><span class="w-6 h-6 rounded-full bg-gray-200 text-gray-500 flex items-center justify-center text-xs font-bold">2</span><span class="text-sm flex-1">時段分割各半</span><span class="text-xs px-2 py-0.5 rounded-full bg-fju-yellow/20 text-fju-yellow">78%</span></div><div class="flex items-center gap-2 p-2 rounded bg-white border border-gray-100"><span class="w-6 h-6 rounded-full bg-gray-200 text-gray-500 flex items-center justify-center text-xs font-bold">3</span><span class="text-sm flex-1">吉他社延後隔天</span><span class="text-xs px-2 py-0.5 rounded-full bg-fju-yellow/20 text-fju-yellow">72%</span></div></div>','<div class="mt-3 p-3 rounded-fju bg-fju-blue/5 border border-fju-blue/10 animate-fadeIn"><i class="fas fa-lightbulb text-fju-yellow mr-1"></i><span class="text-xs">AI 推薦方案一（85%），雙方確認後自動更新。</span></div>'],900)}
...
function checkCreditWarnings(){
  ...
  setTimeout(()=>{
    const warnings=[
      {club:'街舞社',score:62,trend:'down',alert:'連續3月下降，接近停權門檻(60)'},
      {club:'桌遊社',score:71,trend:'down',alert:'本月因逾期歸還器材扣5分'},
      {club:'登山社',score:55,trend:'critical',alert:'已低於60分門檻！建議立即處理'},
    ];
...
function showAdminDashboard(){
  alert('跳轉至審核統計看板...');
  window.location.href='/module/reports?role=admin';
}

function showProfessorAnalysis(){
  alert('顯示社團表現分析雷達圖...');
  window.location.href='/dashboard?role=professor';
}
