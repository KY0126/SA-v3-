...
const STATUS_LABELS = {
  ...
  pending: '待審核', picked_up: '使用中', confirmed: '已確認', negotiating: '協商中',
  cancelled: '已取消', conflicted: '衝突中',
};
...
// ─── Build day map — stores uid refs into _detailStore ───────────────────
function buildDayMap() {
  ...
  if (typeFilter === 'all' || typeFilter === 'activity') {
    allActivities.forEach(a => {
      ...
      const uid = 'a' + seq++;
      _detailStore[uid] = { type: 'activity', raw: a };
      const chipLabel = (a.unit_code ? '[' + a.unit_code + '] ' : '') + a.activity_name;
      add(a.event_date, { type: 'activity', uid, title: a.activity_name, chipLabel, sub: STATUS_LABELS[a.status] || a.status, date: a.event_date });
    });
  }
  if (typeFilter === 'all' || typeFilter === 'equipment') {
    allLoans.forEach(l => {
      ...
      if (l.borrow_date) {
        const uid = 'e' + seq++;
        _detailStore[uid] = { type: 'equipment', raw: l };
        const chipLabel = (l.unit_code ? '[' + l.unit_code + '] ' : '') + names;
        add(l.borrow_date, { type: 'equipment', uid, title: names, chipLabel, sub: STATUS_LABELS[l.status] || l.status, date: l.borrow_date });
      }
      if (l.expected_return_date && l.expected_return_date !== l.borrow_date) {
        const uid2 = 'e' + seq++;
        _detailStore[uid2] = { type: 'equipment', raw: l };
        add(l.expected_return_date, { type: 'equipment', uid: uid2, title: names + '（歸還）', chipLabel: names + '（歸還）', sub: '歸還日', date: l.expected_return_date });
      }
    });
  }
  if (typeFilter === 'all' || typeFilter === 'venue') {
    allVenues.forEach(v => {
      ...
      if (!dateKey) return;
      const uid = 'v' + seq++;
      _detailStore[uid] = { type: 'venue', raw: v };
      const venueName = v.venue?.name || v.venue_name || '場地預約';
      const chipLabel = (v.unit_code ? '[' + v.unit_code + '] ' : '') + venueName;
      add(dateKey, { type: 'venue', uid, title: venueName, chipLabel, sub: STATUS_LABELS[v.status] || v.status, date: dateKey });
    });
  }
  if (typeFilter === 'all' || typeFilter === 'event') {
    allCalEvents.forEach(e => {
      if (!e.date) return;
      const uid = 'c' + seq++;
      _detailStore[uid] = { type: 'event', raw: e };
      add(e.date, { type: 'event', uid, title: e.title, chipLabel: e.title, sub: e.type || '', date: e.date });
    });
  }
  return map;
}
...
function renderDetailCard(entry) {
  ...
  const TYPE_LABEL = { activity: '活動申請', equipment: '器材借用', venue: '場地預約', event: '行事曆事件' };
  let rows = [];
  ...
  if (entry.type === 'activity') {
    ...
    rows = [
      ['序號',     a.serial_no || '—'],
      ['活動名稱', a.activity_name],
      ['單位代碼', a.unit_code || '—'],
      ['日期',     a.event_date],
      ['時間',     (a.start_time || '—') + ' – ' + (a.end_time || '—')],
      ['預計人數', a.expected_participants ? a.expected_participants + ' 人' : '—'],
      ['預計場地', a.venue_description || '—'],
      ['申請預算', a.budget_requested != null ? '$ ' + parseInt(a.budget_requested).toLocaleString() : '—'],
      ['活動目的', a.purpose || '—'],
      ['狀態',     STATUS_LABELS[a.status] || a.status],
    ];
    ...
    rows = [
      ...
      ['單位代碼', l.unit_code || '—'],
      ['借用開始', l.borrow_date || '—'],
      ['預計歸還', l.expected_return_date || '—'],
      ['借用事由', l.purpose || '—'],
      ['狀態',     STATUS_LABELS[l.status] || l.status],
    ];
  } else if (entry.type === 'venue') {
    const v = entry.raw;
    const dateKey = v.booking_date || v.reservation_date || '—';
    rows = [
      ['序號',     v.serial_no || '—'],
      ['場地',     v.venue?.name || v.venue_name || '—'],
      ['單位代碼', v.unit_code || '—'],
      ['預約日期', dateKey],
      ['時間',     (v.start_time || '—') + ' – ' + (v.end_time || '—')],
      ['預估人數', v.expected_participants ? v.expected_participants + ' 人' : '—'],
      ['用途說明', v.purpose || '—'],
      ['狀態',     STATUS_LABELS[v.status] || v.status],
    ];
  } else {
    const e = entry.raw;
    rows = [
  ...
  const rowsHtml = rows.map(([label, val]) =>
    `<div class="flex gap-2 text-xs py-0.5">
      <span class="text-gray-400 shrink-0 w-16">${label}</span>
      <span class="text-gray-700 font-medium break-all">${val}</span>
    </div>`
  ).join('');

  return `<div class="p-3 rounded-fju" style="background:${c.bg};border-left:3px solid ${c.border}">
    <div class="font-bold text-xs mb-2 flex items-center gap-1.5" style="color:${c.text}">
...
function renderCalendar() {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    html += `<div class="p-1.5 min-h-[90px] border border-gray-100 cursor-pointer hover:bg-fju-yellow/5 transition-all ${isToday ? 'bg-fju-blue/5 ring-2 ring-inset ring-fju-blue/30' : ''}" onclick="openDayModal('${ds}')">`;
    ...
    if (dayItems.length > 3) html += `<span class="text-[9px] text-gray-400">+${dayItems.length - 3}</span>`;
    html += '</div>';

    dayItems.slice(0, 3).forEach(item => {
      const c = COLORS[item.type];
      html += `<div class="text-[9px] px-1 py-0.5 rounded mb-0.5 truncate cursor-pointer hover:opacity-80"
        style="background:${c.bg};color:${c.text};border-left:2px solid ${c.border}"
        onclick="event.stopPropagation();showCalDetail('${item.uid}')"
        title="${item.chipLabel}">${item.chipLabel}</div>`;
    });
    html += '</div>';
  }
  html += '</div>';
...
function openDayModal(ds) {
  ...
  const items = map[ds] || [];
  document.getElementById('day-modal-body').innerHTML = items.length
    ? items.map(item => renderDetailCard(_detailStore[item.uid])).join('')
    : '<p class="text-gray-400 text-center py-4"><i class="fas fa-calendar-check mr-1 text-fju-green"></i>此日無任何事項</p>';
  document.getElementById('day-modal').classList.remove('hidden');
}

// ─── Single item detail popup ────────────────────────────────────────────
function showCalDetail(uid) {
  const entry = _detailStore[uid];
  if (!entry) return;
  document.getElementById('day-modal-date').textContent =
    { activity: '活動申請', equipment: '器材借用', venue: '場地預約', event: '行事曆事件' }[entry.type] + ' 詳情';
  document.getElementById('day-modal-body').innerHTML = renderDetailCard(entry);
  document.getElementById('day-modal').classList.remove('hidden');
}
...
function renderList() {
  ...
  buildDayMap(); // ensure _detailStore is populated
  let items = [];

  if (typeFilter === 'all' || typeFilter === 'activity') {
    allActivities.forEach(a => {
      if (!a.event_date) return;
      const uid = Object.keys(_detailStore).find(k => _detailStore[k].type === 'activity' && _detailStore[k].raw === a);
      items.push({ type: 'activity', uid, title: a.activity_name,
        date: a.event_date,
        col3: a.unit_code || '—',
        col4: (STATUS_LABELS[a.status] || a.status) + (a.start_time ? ' · ' + a.start_time + '–' + a.end_time : ''),
      });
    });
  }
  if (typeFilter === 'all' || typeFilter === 'equipment') {
    allLoans.forEach(l => {
      if (!l.borrow_date) return;
      const uid = Object.keys(_detailStore).find(k => _detailStore[k].type === 'equipment' && _detailStore[k].raw === l);
      const names = (l.details || []).map(d => d.equipment?.name || '').filter(Boolean).join('、') || '器材借用';
      items.push({ type: 'equipment', uid, title: names,
        date: l.borrow_date,
        col3: l.unit_code || '—',
        col4: (STATUS_LABELS[l.status] || l.status) + ' · 至 ' + (l.expected_return_date || '—'),
      });
    });
  }
  if (typeFilter === 'all' || typeFilter === 'venue') {
    allVenues.forEach(v => {
      const dateKey = v.booking_date || v.reservation_date || v.event_date;
      if (!dateKey) return;
      const uid = Object.keys(_detailStore).find(k => _detailStore[k].type === 'venue' && _detailStore[k].raw === v);
      items.push({ type: 'venue', uid, title: v.venue?.name || v.venue_name || '場地預約',
        date: dateKey,
        col3: v.unit_code || '—',
        col4: (STATUS_LABELS[v.status] || v.status) + (v.start_time ? ' · ' + v.start_time + '–' + v.end_time : ''),
      });
    });
  }
  if (typeFilter === 'all' || typeFilter === 'event') {
    allCalEvents.forEach(e => {
      if (!e.date) return;
      const uid = Object.keys(_detailStore).find(k => _detailStore[k].type === 'event' && _detailStore[k].raw === e);
      items.push({ type: 'event', uid, title: e.title, date: e.date, col3: '—', col4: e.type || '—' });
    });
  }
  ...
  document.getElementById('unified-list').innerHTML =
    ...
    '<th class="p-4">類型</th><th class="p-4">名稱 / 器材</th><th class="p-4">日期</th><th class="p-4">單位代碼</th><th class="p-4">狀態 / 時間</th></tr></thead><tbody>' +
    items.map(item => {
      const c = COLORS[item.type];
      const clickAttr = item.uid ? `onclick="showCalDetail('${item.uid}')" style="cursor:pointer"` : '';
      return `<tr class="border-t border-gray-50 hover:bg-fju-yellow/5 transition-all" ${clickAttr}>
        <td class="p-4"><span class="flex items-center gap-1.5 text-xs font-bold" style="color:${c.text}">
          <span class="w-2 h-2 rounded-full shrink-0" style="background:${c.dot}"></span>${TYPE_LABEL[item.type]}
        </span></td>
        <td class="p-4 font-medium text-fju-blue">${item.title}</td>
        <td class="p-4 text-xs text-gray-500">${item.date}</td>
        <td class="p-4 text-xs text-gray-500">${item.col3}</td>
        <td class="p-4 text-xs text-gray-400">${item.col4}</td>
      </tr>`;
    }).join('') + '</tbody></table>';
}
