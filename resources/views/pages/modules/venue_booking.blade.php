...
<div class="space-y-6">
  ...
  {{-- Stats --}}
  <div class="grid md:grid-cols-4 gap-4" id="venue-stats">
    <div class="p-8 text-center text-gray-400"><i class="fas fa-spinner fa-spin"></i></div>
  </div>
  ...
  <div class="flex gap-2 flex-wrap">
    ...
    <button onclick="setVbView('review')" id="vb-tab-review" class="vb-tab px-4 py-1.5 rounded-fju text-xs bg-gray-100 text-gray-500 font-medium"><i class="fas fa-clipboard-check mr-1 text-fju-yellow"></i>審核申請</button>
    @endif
  </div>
  ...
  <div id="vb-rank-view" class="hidden bg-white rounded-fju-lg shadow-sm border border-gray-100 overflow-hidden">
    <div class="px-5 py-3 border-b border-gray-100">
      <h3 class="font-bold text-fju-blue text-sm"><i class="fas fa-trophy mr-2 text-fju-yellow"></i>熱門場地借用排行榜（近 3 個月）</h3>
    </div>
  ...
  @if(in_array($role ?? 'student', ['admin']))
  <div id="vb-review-view" class="hidden space-y-4">
    <div class="flex gap-2 flex-wrap">
      @foreach(['all'=>'全部','pending'=>'待審核','approved'=>'已核准','returned'=>'已退件','rejected'=>'已拒絕'] as $k=>$v)
      <button onclick="filterVenueBookings('{{$k}}')" class="vb-bk-filter px-4 py-2 rounded-fju text-sm {{ $k==='all'?'bg-fju-blue text-white':'bg-gray-100 text-gray-500' }}" data-f="{{$k}}">{{$v}}</button>
      @endforeach
    </div>
    <div class="bg-white rounded-fju-lg shadow-sm border border-gray-100 overflow-hidden">
      <div id="vb-booking-list">
        <div class="p-8 text-center text-gray-400"><i class="fas fa-spinner fa-spin mr-2"></i>載入中...</div>
      </div>
    </div>
  </div>
  ...
  {{-- Admin: Venue Booking Detail / Review Modal --}}
  @if(in_array($role ?? 'student', ['admin']))
  <div id="vb-booking-detail-modal" class="hidden fixed inset-0 bg-black/50 z-50 flex items-center justify-center">
    <div class="bg-white rounded-fju-lg p-6 w-full max-w-lg mx-4 shadow-2xl max-h-[90vh] overflow-y-auto">
      <div class="flex items-center justify-between mb-4">
        <h3 class="font-bold text-fju-blue text-lg"><i class="fas fa-search mr-2 text-fju-yellow"></i>場地預約詳情</h3>
        <button onclick="closeVbBookingModal()" class="text-gray-400 hover:text-gray-600"><i class="fas fa-times"></i></button>
      </div>
      <div id="vb-booking-detail-body" class="space-y-3 text-sm"></div>
      <div id="vb-booking-review-actions" class="hidden flex gap-2 mt-4"></div>
    </div>
  </div>

  {{-- Admin: Venue Booking Return Reason Modal --}}
  <div id="vb-return-modal" class="hidden fixed inset-0 bg-black/50 z-[60] flex items-center justify-center">
    <div class="bg-white rounded-fju-lg p-6 w-full max-w-md mx-4 shadow-2xl">
      <div class="flex items-center justify-between mb-4">
        <h3 class="font-bold text-yellow-600 text-lg"><i class="fas fa-undo-alt mr-2"></i>退件場地預約</h3>
        <button onclick="closeVbReturnModal()" class="text-gray-400 hover:text-gray-600"><i class="fas fa-times"></i></button>
      </div>
      <p class="text-sm text-gray-500 mb-3">退件原因為必填，申請人將收到通知並可修改後重新送出。</p>
      <textarea id="vb-return-reason" rows="4" placeholder="請輸入退件原因..."
      ...
      <div id="vb-return-error" class="hidden mt-2 text-red-500 text-xs"><i class="fas fa-exclamation-circle mr-1"></i>退件原因不可空白</div>
      <div class="flex gap-2 mt-4">
        <button onclick="confirmVbReturn()" class="flex-1 py-2.5 rounded-fju bg-yellow-400 text-fju-blue text-sm font-bold hover:bg-yellow-500"><i class="fas fa-undo mr-1"></i>確認退件</button>
        <button onclick="closeVbReturnModal()" class="flex-1 py-2.5 rounded-fju border border-gray-200 text-sm text-gray-500">取消</button>
      </div>
    </div>
  </div>

  {{-- Admin: Venue Booking Reject Reason Modal --}}
  <div id="vb-reject-modal" class="hidden fixed inset-0 bg-black/50 z-[60] flex items-center justify-center">
    <div class="bg-white rounded-fju-lg p-6 w-full max-w-md mx-4 shadow-2xl">
      <div class="flex items-center justify-between mb-4">
        <h3 class="font-bold text-red-600 text-lg"><i class="fas fa-times-circle mr-2"></i>拒絕場地預約</h3>
        <button onclick="closeVbRejectModal()" class="text-gray-400 hover:text-gray-600"><i class="fas fa-times"></i></button>
      </div>
      <p class="text-sm text-gray-500 mb-3">拒絕原因為必填，申請人將收到通知。</p>
      <textarea id="vb-reject-reason" rows="4" placeholder="請輸入拒絕原因..."
        class="w-full px-4 py-2 rounded-fju border border-gray-200 text-sm resize-none focus:border-red-400 outline-none"></textarea>
      <div id="vb-reject-error" class="hidden mt-2 text-red-500 text-xs"><i class="fas fa-exclamation-circle mr-1"></i>拒絕原因不可空白</div>
      <div class="flex gap-2 mt-4">
        <button onclick="confirmVbReject()" class="flex-1 py-2.5 rounded-fju bg-red-500 text-white text-sm font-bold hover:bg-red-600"><i class="fas fa-times mr-1"></i>確認拒絕</button>
        <button onclick="closeVbRejectModal()" class="flex-1 py-2.5 rounded-fju border border-gray-200 text-sm text-gray-500">取消</button>
      </div>
    </div>
  </div>
  @endif
        ...
        <div class="grid md:grid-cols-2 gap-4">
          <div class="bg-fju-blue/5 rounded-fju p-4 border border-fju-blue/10">
            <div class="flex items-center gap-2 mb-2">
              <div class="w-8 h-8 rounded-fju bg-fju-blue flex items-center justify-center text-white text-sm font-bold">A</div><span class="font-bold text-fju-blue" id="cd-party-a">-</span>
            </div>
          ...
          <div class="bg-fju-yellow/5 rounded-fju p-4 border border-fju-yellow/10">
            <div class="flex items-center gap-2 mb-2">
              <div class="w-8 h-8 rounded-fju bg-fju-yellow flex items-center justify-center text-fju-blue text-sm font-bold">B</div><span class="font-bold text-fju-blue" id="cd-party-b">-</span>
            </div>
...
<script>
  ...
  function setVbView(v) {
    ...
    document.getElementById('vb-review-view')?.classList.toggle('hidden', v !== 'review');
    if (v === 'rank') renderVbRanking();
    if (v === 'history') renderVbHistory();
    if (v === 'review') loadVenueBookings();
  }
  ...
  function renderVbHistory() {
    const sorted = [...allReservations].sort((a, b) => {
      const da = a.reservation_date || '',
        db = b.reservation_date || '';
  ...
  (function() {
    Promise.all([
      ...
      if (sel) {
        ...
        allVenues.filter(v => v.status === 'available').forEach(v => {
  ...
  function checkConflict() {
    ...
    if (conflicting.length > 0) {
      ...
      if (submitBtn) {
        ...
        submitBtn.className = 'w-full py-3 mt-4 rounded-fju bg-gray-200 text-gray-400 text-sm font-bold cursor-not-allowed';
      }
    } else {
      warnEl.classList.add('hidden');
      if (submitBtn) {
        submitBtn.disabled = false;
        submitBtn.className = 'w-full btn-yellow py-3 mt-4';
      }
    }
  }

  function renderVenueStats() {
    const a = allVenues.filter(v => v.status === 'available').length,
      m = allVenues.filter(v => v.status === 'maintenance').length;
  ...
  function renderActiveConflicts(conflicts) {
    ...
    if (active.length === 0) {
      document.getElementById('active-conflicts').classList.add('hidden');
      return
    }
  ...
  function renderConflicts() {
    ...
    if (data.length === 0) {
      document.getElementById('conflict-list').innerHTML = '<div class="p-8 text-center text-gray-400"><i class="fas fa-check-circle mr-2 text-fju-green"></i>目前無衝突</div>';
      return
    }
    const sc = {
      'pending': 'bg-fju-red/10 text-fju-red',
      'negotiating': 'bg-fju-yellow/20 text-fju-yellow',
      'resolved': 'bg-fju-green/10 text-fju-green'
    };
    const sl = {
      'pending': '待處理',
      'negotiating': '協商中',
  ...
  function openDetail(id) {
    ...
    fetch(`/api/conflicts/${id}`).then(r => r.json()).then(res => {
      ...
      if (c.status === 'resolved') {
        ...
        document.getElementById('cd-resolution-msg').textContent = '衝突已解決！雙方均已確認。'
      } else {
  ...
  function updateConfirmBtn(party, confirmed, resolved) {
    ...
    if (confirmed || resolved) {
      ...
      btn.innerHTML = '<i class="fas fa-check-circle mr-1"></i>確認協商結果';
      btn.disabled = false
    }
  }
  ...
  function renderChatMessages(msgs) {
    ...
    if (!msgs || msgs.length === 0) {
      el.innerHTML = '<div class="text-center text-gray-400 text-xs py-4"><i class="fas fa-comments mr-1"></i>尚無對話</div>';
      return
    }
    ...
    el.innerHTML = msgs.map(m => {
      ...
      return `<div class="flex ${isFirst?'justify-start':'justify-end'}"><div class="max-w-[70%] px-3 py-2 rounded-fju text-sm ${senderColorMap[m.sender]}"><div class="text-[10px] font-bold mb-1">${m.sender}</div><div>${m.message}</div><div class="text-[9px] text-gray-400 mt-1">${new Date(m.timestamp).toLocaleTimeString('zh-TW')}</div></div></div>`
    }).join('');
  ...
  function sendChat() {
    ...
    fetch(`/api/conflicts/${currentConflictId}/chat`, {
      method: 'POST',
      headers: {
  ...
  function sendEmail(party) {
    ...
    fetch(`/api/conflicts/${currentConflictId}/send-email`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Accept': 'application/json'
  ...
  function confirmResolution(party) {
    ...
    fetch(`/api/conflicts/${currentConflictId}/confirm`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Accept': 'application/json'
      ...
      if (res.both_confirmed) {
        ...
        fetch('/api/conflicts').then(r => r.json()).then(res2 => {
          allConflicts = res2.data || [];
          renderConflictStats();
          renderConflicts();
          renderActiveConflicts(allConflicts)
        });
  ...
  function inviteParticipant() {
    document.getElementById('invite-panel').classList.toggle('hidden')
  }
  ...
  function closeDetailModal() {
    document.getElementById('conflict-detail-modal').classList.add('hidden');
    clearInterval(timerInterval)
  }

  function showToast(msg) {
    const t = document.createElement('div');
    t.className = 'fixed bottom-6 right-6 bg-fju-blue text-white px-5 py-3 rounded-fju shadow-lg text-sm z-[100]';
    t.textContent = msg;
  ...
  // ─── Admin: Venue Booking Review ─────────────────────────────────────────
  let allVenueBookings = [], currentVbBookingFilter = 'all', currentVbBookingId = null;

  function loadVenueBookings() {
    document.getElementById('vb-booking-list').innerHTML = '<div class="p-8 text-center text-gray-400"><i class="fas fa-spinner fa-spin mr-2"></i>載入中...</div>';
    fetch('/api/venue-bookings').then(r => r.json()).then(res => {
      allVenueBookings = res.data || [];
      renderVenueBookings();
    }).catch(() => {
      document.getElementById('vb-booking-list').innerHTML = '<div class="p-8 text-center text-red-400">載入失敗，請重新整理</div>';
    });
  }

  function vbBookingStatusBadge(s) {
    return ({
      ...
      cancelled: '<span class="px-2 py-1 rounded-fju text-xs bg-gray-100 text-gray-500">已取消</span>',
      conflicted:'<span class="px-2 py-1 rounded-fju text-xs bg-orange-100 text-orange-600">衝突</span>',
    })[s] || `<span class="text-xs text-gray-400">${s}</span>`;
  }

  function renderVenueBookings() {
    const data = currentVbBookingFilter === 'all'
      ? [...allVenueBookings]
      : allVenueBookings.filter(b => b.status === currentVbBookingFilter);
    if (!data.length) {
      document.getElementById('vb-booking-list').innerHTML = '<div class="p-8 text-center text-gray-400">目前沒有符合的預約申請</div>';
      return;
    }
    document.getElementById('vb-booking-list').innerHTML =
      '<table class="w-full text-sm"><thead class="bg-gray-50"><tr class="text-left text-xs text-gray-400">' +
      '<th class="p-4">序號</th><th class="p-4">場地</th><th class="p-4">申請人</th><th class="p-4">日期</th><th class="p-4">時段</th><th class="p-4">狀態</th><th class="p-4">操作</th>' +
      '</tr></thead><tbody>' +
      data.map(b => `<tr class="border-t border-gray-50 hover:bg-gray-50">
        <td class="p-4 text-xs text-gray-400 font-mono">${b.serial_no}</td>
        <td class="p-4 font-medium text-fju-blue">${b.venue?.name || '場地 #' + b.venue_id}</td>
        <td class="p-4 text-xs">${b.applicant?.name || '申請人 #' + b.applicant_id}</td>
        <td class="p-4 text-xs">${b.booking_date}</td>
        <td class="p-4 text-xs text-gray-500">${b.start_time}–${b.end_time}</td>
        <td class="p-4">${vbBookingStatusBadge(b.status)}</td>
        <td class="p-4"><button onclick="openVbBookingDetail(${b.id})" class="btn-blue px-3 py-1 text-xs">詳情</button></td>
      </tr>`).join('') + '</tbody></table>';
  }

  function filterVenueBookings(f) {
    currentVbBookingFilter = f;
    document.querySelectorAll('.vb-bk-filter').forEach(b => {
      b.classList.remove('bg-fju-blue', 'text-white');
      b.classList.add('bg-gray-100', 'text-gray-500');
    });
    const btn = document.querySelector(`.vb-bk-filter[data-f="${f}"]`);
    btn?.classList.add('bg-fju-blue', 'text-white');
    btn?.classList.remove('bg-gray-100', 'text-gray-500');
    renderVenueBookings();
  }

  function openVbBookingDetail(id) {
    currentVbBookingId = id;
    const b = allVenueBookings.find(x => x.id === id);
    if (!b) return;
    document.getElementById('vb-booking-detail-body').innerHTML = `
      <div class="p-3 rounded-fju bg-fju-bg space-y-1">
        <div class="flex items-center justify-between">
          <span class="text-xs text-gray-400">序號</span>
          <span class="font-mono font-bold text-fju-blue">${b.serial_no}</span>
        </div>
        <div class="flex items-center justify-between">
          <span class="text-xs text-gray-400">狀態</span>${vbBookingStatusBadge(b.status)}
        </div>
      </div>
      <div><span class="text-xs text-gray-400">場地</span><div class="font-bold text-fju-blue">${b.venue?.name || '場地 #' + b.venue_id}</div></div>
      <div><span class="text-xs text-gray-400">申請人</span><div class="text-sm">${b.applicant?.name || '申請人 #' + b.applicant_id}</div></div>
      <div class="grid grid-cols-2 gap-2 text-xs text-gray-500">
        <div><i class="fas fa-calendar mr-1 text-fju-yellow"></i>${b.booking_date}</div>
        <div><i class="fas fa-clock mr-1 text-fju-yellow"></i>${b.start_time} – ${b.end_time}</div>
        <div><i class="fas fa-users mr-1 text-fju-yellow"></i>預估 ${b.expected_participants || 0} 人</div>
        ${b.activity_application_id ? `<div><i class="fas fa-link mr-1 text-fju-yellow"></i>關聯申請 #${b.activity_application_id}</div>` : ''}
      </div>
      ${b.purpose ? `<div><span class="text-xs text-gray-400">用途說明</span><div class="text-sm">${b.purpose}</div></div>` : ''}
      ${b.reject_reason ? `<div class="p-3 rounded-fju bg-red-50 border border-red-100"><span class="text-xs text-red-400">退件/拒絕原因</span><div class="text-sm text-red-600">${b.reject_reason}</div></div>` : ''}
    `;
    const actEl = document.getElementById('vb-booking-review-actions');
    if (b.status === 'pending') {
      actEl.innerHTML = `
        <button onclick="doVbApprove()" class="flex-1 py-2.5 rounded-fju bg-green-500 text-white text-sm font-bold hover:bg-green-600"><i class="fas fa-check mr-1"></i>核准</button>
        <button onclick="doVbReturn()" class="flex-1 py-2.5 rounded-fju bg-yellow-400 text-fju-blue text-sm font-bold hover:bg-yellow-500"><i class="fas fa-undo mr-1"></i>退件</button>
        <button onclick="doVbReject()" class="flex-1 py-2.5 rounded-fju bg-red-500 text-white text-sm font-bold hover:bg-red-600"><i class="fas fa-times mr-1"></i>拒絕</button>
      `;
      actEl.classList.remove('hidden');
    } else {
      actEl.classList.add('hidden');
    }
    document.getElementById('vb-booking-detail-modal').classList.remove('hidden');
  }

  function closeVbBookingModal() {
    document.getElementById('vb-booking-detail-modal').classList.add('hidden');
  }

  function doVbApprove() {
    if (!confirm('確認核准此場地預約？')) return;
    fetch(`/api/venue-bookings/${currentVbBookingId}/approve`, {
      method: 'POST',
  ...
  function doVbReturn() {
    document.getElementById('vb-return-reason').value = '';
    document.getElementById('vb-return-error').classList.add('hidden');
    document.getElementById('vb-return-modal').classList.remove('hidden');
  }

  function closeVbReturnModal() {
    document.getElementById('vb-return-modal').classList.add('hidden');
  }

  function confirmVbReturn() {
    const reason = document.getElementById('vb-return-reason').value.trim();
    if (!reason) {
      document.getElementById('vb-return-error').classList.remove('hidden');
      document.getElementById('vb-return-reason').focus();
      return;
    }
    document.getElementById('vb-return-error').classList.add('hidden');
    fetch(`/api/venue-bookings/${currentVbBookingId}/return`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
  ...
  function doVbReject() {
    document.getElementById('vb-reject-reason').value = '';
    document.getElementById('vb-reject-error').classList.add('hidden');
    document.getElementById('vb-reject-modal').classList.remove('hidden');
  }

  function closeVbRejectModal() {
    document.getElementById('vb-reject-modal').classList.add('hidden');
  }

  function confirmVbReject() {
    const reason = document.getElementById('vb-reject-reason').value.trim();
    if (!reason) {
      document.getElementById('vb-reject-error').classList.remove('hidden');
      document.getElementById('vb-reject-reason').focus();
      return;
    }
    document.getElementById('vb-reject-error').classList.add('hidden');
    fetch(`/api/venue-bookings/${currentVbBookingId}/reject`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
      body: JSON.stringify({ reviewer_id: 1, reason }),
    }).then(r => r.json()).then(res => {
  ...
  function quickBook(n) {
    if (!IS_ADMIN) openBookingModal();
  }
    ...
    fetch('/api/reservations', {
      ...
      .then(res => {
        ...
        if (res.success) {
          ...
          fetch('/api/reservations').then(r => r.json()).then(r2 => {
            allReservations = r2.data || [];
            renderVenues();
          });
        } else {
          ...
          if (submitBtn) {
            submitBtn.disabled = false;
            submitBtn.innerHTML = '<i class="fas fa-paper-plane mr-2"></i>送出預約申請';
            submitBtn.className = 'w-full btn-yellow py-3 mt-4';
          }
        }
      })
