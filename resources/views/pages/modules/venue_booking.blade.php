...
fetch('/api/reservations',{method:'POST',headers:{'Content-Type':'application/json','Accept':'application/json'},body:JSON.stringify(d)}).then(r=>r.json()).then(res=>{const b=document.getElementById('booking-result');b.classList.remove('hidden');if(res.success){b.className='mt-3 p-3 rounded-fju text-sm bg-fju-green/10 text-fju-green';b.innerHTML='<i class="fas fa-check-circle mr-1"></i> '+res.message+' (階段：'+res.stage+')'}else{b.className='mt-3 p-3 rounded-fju text-sm bg-fju-yellow/20 text-fju-blue';b.innerHTML='<i class="fas fa-exclamation-triangle mr-1"></i> '+res.message}})}
</script>
@endsection
